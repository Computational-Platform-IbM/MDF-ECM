...
def max_mdf(ln_conc, dg0):  
    ...
    ln_conc[i_rATP] = np.exp(ln_conc[i_Pi]) * cH * np.exp( (dGatp - dGatp0) / (R*T))
    
    #set dg_prime as a function of dg0_prime and variables ln_conc
    dg_prime = dg0 + R*T * stoich.T @ ln_conc
    
    #the optimization target is the lowest driving force of the pathway: this should be increased (so lowered)
    mdf = max(dg_prime)
    return mdf

#import pathway
(reactions, compounds, element_balance, 
 fixed_c, deltaGf0, S_netR, stoich, rel_flux) = importpath('\\glu_to_acetate_ratios_new.xlsx') #\\glu_to_acetic_butyric.xlsx') #\\glu_to_ac_e-bfc_ratios.xlsx')  # 

#get number of compounds and reactions in pathway
Nc, Nr = stoich.shape

#stoich = stoich/stoich[0,0]
...
#     cH = 10**-pH
#     p_ratios =  cH * np.exp((dGatp - dGatp0) / (R*T))
...
def con_NADPH(ln_conc):
    i_rNADPH = compounds.index('NADPH')    
...
#call minimizer
res = minimize(max_mdf, conc0, args=(dg0), method='SLSQP', 
...
dg_prime_opt = dg0 + R*T * stoich.T @ res.x
...
conditions += f'\t Pi-pool = {maxPi:.2e} M \t CoA-pool = {maxCoA:.2e} M'.expandtabs()
...
# ax.title.set_text('Cumulative $\Delta$G throughout the pathway')
# ax.plot([''] + reactions, sumdg0, 'r-', label='dG0 [kJ/mol]', alpha=0.4)
# ax.plot([''] + reactions, sumdg_opt, 'b-', label = "dG' optimized [kJ/mol]")
# ax.grid()
# ax.set_ylabel('Cumulative $\Delta$G [kJ/mol]')
# ax.set_xlabel('Reactions')
# ax.set_xlim(0, len(reactions))
# ax.set_xticklabels(reactions, fontsize=8, rotation=90)
# ax.legend()
